...
```
1. This is the default weight for all editing operations in the error model:
DEFAULT_WEIGHT=10

1. Edit distanse for the Levenshtein error model:
EDIT_DISTANCE=2

1. Define whether we allow changes to the initial letter(s) in the error model,
1. possible values are:
1. * no    - no longer string edits = only the default, letter-based error model
1. * txt   - use only the txt file as source
1. * regex - use only the regex file as source
1. * both  - use both the txt and regex files as sources
1. NB!!! Setting this to anything but 'no' will greatly increase the size and
1. search space of the error model, and thus make it much, much slower. Make sure
1. you TEST the resulting error model properly and thoroughly, both for speed
1. and suggestion quality.
INITIAL_EDITS=no

1. Variable to define whether to enable edits of longer strings (as opposed to
1. single letters). Possible values are:
1. * no    - no longer string edits = only the default, letter-based error model
1. * txt   - use only the txt file as source
1. * regex - use only the regex file as source
1. * both  - use both the txt and regex files as sources
STRING_EDITS=txt
1. Variable to specify the edit distance for the regex
1. version of the strings file. The total edit distance for those operations is
1. this value multiplied with the value of the DEFAULT_EDIT_DIST variable.
STRING_REGEX_EDIT_DISTANCE=2

1. Variable to define whether to enable edits of word-final strings (as opposed
1. to single letters). Possible values are:
1. * no    - no longer string edits = only the default, letter-based error model
1. * txt   - use only the txt file as source
1. * regex - use only the regex file as source
1. * both  - use both the txt and regex files as sources
...
1. Variable to define whether to enable whole-word replacements. Possible values:
1. - yes
1. - no
...
```
    1. Inclusions: this is the real alphabet definition:
a
á	5
b
c
č	6
    ...
    1. Transition pairs + weight - section separator:
@@

    1. Transition pair specifications:
a	á	4
á	a	4
```
