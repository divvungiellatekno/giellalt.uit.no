...
* SGM letter meeting Tuesday 9:30 with **Maja, Thomas**
    - Sorry, that meeting slipped off. Retry this week.
...
* continue to write a proofing API & implementation specification
    - Voikko is coming up as a candidate for the first version of our HFST-based
   prooing tools. Application API is already done, including grammar checking,
   and it has a quite good abstraction of different layers as it is. There is
   still some work to do, but the first running demo of a Finnish speller made
   of HFST and Voikko is already done.
* name db/risten.no
...
* [fix bugs!](http://giellatekno.uit.no/bugzilla)
* other things:
    - still more HFST debugging and bug reporting
...
[Voikko](http://voikko.sf.net) will be our near future proofing solution, using 
HFST as the language technology core.
...
The next meeting is 23.11.2009, 9.30 Norwegian time.
