...
* other:
    - been working on static i18n divvun site
...
* add an Oahpa clock excercise
    - installed Oahpa locally
    - started to create two new Oahpa-variants:
        - Sandbox-Oahpa for **Trond, Lene** further development
        - Train-the-trainer-Oahpa for the winter workshop in Bodø 
...
* incorporate the passives into the last version of the sme:nob
    - todo: advice from **Lene** is needed
...
* package and deliver `sma` proofing tools (with **Tomi**)
    - worked on Polderland downloads
...
* [fix bugs!](http://giellatekno.uit.no/bugzilla)
    - not done
* other tasks:
    - a lot of bureaucratic work with Norwegian and Finnish government agencies
    - started to turn the iCal feature in our meeting notes into a real Forrest
   plugin. This is in preparation to an upgrade that should streamline the
   meeting minutes quite a bit, by doing away with the task lists at the
   beginning and the end - all will be maintained directly in the memo body.
   This will also save me a lot of repeating work.
...
* make Lene et al run the gtsetup.sh script
    - Awaiting her new machine.
* tts startup (with **Sjur**)
    - Date fixed, we are on "go", starting September 1st.
* `sma` proper noun grammar (with the rest of the sma gang)
    - Nothing here last week.
* [fix bugs!](http://giellatekno.uit.no/bugzilla).

# Oahpa!

**Ciprian** now able to install Oahpa! locally. He is making three versions: the
official public version, a developer sandbox for playing with new features, new
languages, etc, and a minimal version for Bodø. The Bodø version will contain
only two plays: MorfaS and Lexa and should be configurable for adding new
language pairs, and it should contain a login and perhaps a file upload
...
* run the gtsetup script (**Lene, Linda**)
...
The next meeting is 31.8.2009, 9.30 Norwegian time.
