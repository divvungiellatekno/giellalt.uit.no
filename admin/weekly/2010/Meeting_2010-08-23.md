...
* get all the latest Sámi Parliament protocolls into our repository
    - Found protocols in nob-sme versions up to 2005, have added most of them.
   Protocols from 2006 up to the second protocol in 2010 don't seem to have
   been translated, so they are lacking.
...
* `sma` seminar in August/September:
    ...
    - augment the  dictionary DTD with oahpa features and requirements
        - waiting for Sjur's counterarguments (**Trond** and **Lene** agree on the intermediate solution proposed by **Cip**)
...
* read and answer the newsgroups messages
    - reading Performace Grammar and Nunberg's Linguistics of Punctuation
* update corpora (both monolingual and parallel) for Oslo (Glossa)
    - check the correctness of parallelity between sme and nob files
        - after setting up the whole conversion pipeline
...
* corpus infra:
    ...
    - make the conversion scripts work with the new svn repository
        - adjusted paths to scrips after reorganizing script-dir
        - add check for almost empty content: todo
        - add processing of svg files: todo
    - reorganise subdirs as needed
        - created a subdir corpus and moved all (check with **Børre**, **Trond** and **Sjur**) relevant file there
        - ajsusted paths in the analye file, now checking the convert script
* infrastructure
    - test cwb
    - continue with restructuring and cleaning the script catalogue as suggested
   in the newsgroup
        - done for corpus-related stuff
    - transform sme-lexC files into XML format
        - apparently, there is a script that does exactly that (ask **Tomi**)
...
**ongoing - with Ove Lorentz - lage oppgaver for gruppearbeid
...
* test the `sma` speller on the gold standard document
    - done, committed, and sent to Thomas and Maja for checking
* add voikko support to our proofing test bench
    - not yet
* add all our Sámi analysers to voikko and test them as spellers
    - not yet
* run speller tests using Hunspell, Voikko, Polderland for our Sámi lexicons
    - not yet
...
* check for a new computer
    - done, it should be here this week
* fix goldstandard files (location)
    - maybe
* test conversion to xml to ensure there are no markup errors
    - just begun
...
* check extra hyphens in the lexc continuations with Thomas
    - Done some, but no meeting with Thomas yet
...
* Northern areas
    ...
    - Awaiting
* MT/Terminology
    - Much work here, both on smenob, finsme and smesma
* [fix bugs!](http://giellatekno.uit.no/bugzilla).
    - Done 2.

# Oahpa!
Status:
* sma-oahpa already started, installation will be done by this friday
* **Ciprian** should add just 15 entires per pos n, v, a
* the interim lexicon format as already proposed via email:
    - the current gt_dict lexicon having each relevant meaning group
   extended with application relevant slots.
...
* get all the latest Sámi Parliament protocolls into our repository (**Børre**)
    - mostly done, protocolls from the period 2006-2010/1 do not seem to be
   translated into `sme` - Børre will check this
* contact the Sámi translators at the government and ministeries to get more
...
**Ciprian** has moved all code relating to `convert2xml.pl` to a subfolder.
Still needs testing especially on victorio.
...
**Sjur** made a budget for risten2 with great help from Ciprian and Trond.
...
**TODO:**
* book lunch, coffee
* make a list of participants (12 + 5 + 3) in `$GTPRIV/` (**Maja**)
    - done
...
The next meeting is 6.9.2010, 09:30 Norwegian time.
